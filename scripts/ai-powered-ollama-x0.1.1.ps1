...
Write-Step "ai-powered + Ollama Launcher v2.3 - Starting up"
...
if (-not $AutoStart) {
    Write-Ok "Setup complete. Run with -AutoStart `$true to launch."
    exit 0
}
...
try {
    ...
    Write-Warn "Model check skipped. Continuing..."
}
...
# --- Vite (FIXED: different log files) ---
...
if ($Ngrok) {
    ...
    if (Get-Command ngrok -EA SilentlyContinue) {
        # ngrok logic (simplified)
        Write-Ok "ngrok launched (check logs for URL)"
    } else {
        Write-Warn "ngrok not installed. Skip public URL."
    }
}
...
Write-Host "`nLogs folder: $LogDir" -ForegroundColor DarkGray
